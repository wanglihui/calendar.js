...
;(function(){
    ...
    function Calendar(options) {
        ...
        this.isShowWeekDayName = options.isShowWeekDayName;
        ...
        if (!this.show) {
            ...
            this.show = function(data) {
                var calObj = document.getElementById(self.containerId);
                if (!calObj) {
                    document.write('<div id="'+self.containerId+'"></div>');
                }

                document.getElementById(self.containerId).innerHTML = data;
                return;
            };
        }
    }
    ...
    Calendar.prototype.oneMonth = function(month, year) {
        ...
        for(var i=1; i<=days; i++) {
            ...
            //补最后空格
            if (i == days) {
                var endSpaces = (7-lastDay-1)
                for(var j=0; j< endSpaces; j++) {
                    oneWeek.push(self.renderDay(0, month, year));
                    idx = idx + 1;
                }
